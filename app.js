document.addEventListener("DOMContentLoaded", () => {
  ...
  function getSmokingText(value) {
    if (value === "past") return "과거흡연";
    if (value === "current") return "현재흡연";
    return "비흡연";
  }

  function getEmergencyCount(formElement) {
    if (!formElement) return 0;
    return Array.from(formElement.querySelectorAll(".emergency")).filter(
      (item) => item.checked
    ).length;
  }
  ...
  function calculateAdvancedRiskProbability(data) {
    ...
    const hbLow = (data.male === 1 && data.hb < 13.0) || (data.male === 0 && data.hb < 12.0) ? 1 : 0;
  ...
  function getBasicRiskFactors(data) {
    const factors = [];
    if (data.age >= 70) factors.push(["연령", "70세 이상"]);
    else if (data.age >= 60) factors.push(["연령", "60세 이상"]);
    else if (data.age >= 50) factors.push(["연령", "50세 이상"]);
    
    if (data.male) factors.push(["성별", "남성"]);
    if (data.bmi >= 25) factors.push(["BMI", "비만 범위"]);
    if (data.hypertension) factors.push(["고혈압", "의사진단 있음"]);
    if (data.diabetes) factors.push(["당뇨병", "의사진단 있음"]);
    if (data.dyslipidemia) factors.push(["이상지질혈증", "의사진단 있음"]);
    if (data.stroke) factors.push(["뇌졸중", "의사진단 있음"]);
    if (data.smoking === "past") factors.push(["흡연상태", "과거흡연"]);
    if (data.smoking === "current") factors.push(["흡연상태", "현재흡연"]);
    if (data.stress) factors.push(["스트레스", "있음"]);
    if (data.ihdFamily) factors.push(["허혈성심질환 가족력", "있음"]);
  ...
  function getAdvancedRiskFactors(data) {
    ...
    if (data.bmi >= 30) factors.push(["BMI", "비만"]);
    else if (data.bmi >= 25) factors.push(["BMI", "과체중"]);
    
    if (data.sbp >= 140 || data.dbp >= 90 || data.bpMedication) {
      factors.push(["혈압", "고혈압 또는 약 복용"]);
    }
    if ((data.male === 1 && data.hb < 13.0) || (data.male === 0 && data.hb < 12.0)) {
      factors.push(["혈색소", "낮음"]);
    }
    if (data.glu >= 126) factors.push(["공복혈당", "당뇨병 의심"]);
    else if (data.glu >= 100) factors.push(["공복혈당", "공복혈당장애 의심"]);
    
    if (data.hdl < 40) factors.push(["HDL 콜레스테롤", "낮음"]);
    else if (data.hdl < 60) factors.push(["HDL 콜레스테롤", "보통"]);
